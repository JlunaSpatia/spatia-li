...
try:
    ...
    from conf import DB_CONNECTION_STR, SCRAPINGDOG_API_KEY, ZOOM_LEVEL
    print("✅ Configuración cargada correctamente.")
except ImportError:
    print(f"❌ Error crítico: No encuentro 'conf.py'.")
    sys.exit(1)

# ================= 1. LISTA V3: LA MÁS COMPLETA QUE HEMOS HECHO =================
def get_smart_search_terms():
    print("🧠 Cargando lista V3 (Incluye tus nichos perdidos)...")
    return [
        # --- FITNESS & WELLNESS ---
        ...
        "Centro de bienestar", "Detox", # <--- NUEVO (Para KO Urban Detox)

        # --- RETAIL MODA & SNEAKERS ---
        "Tienda de ropa", "Tienda de ropa de mujer", "Tienda de ropa de hombre", 
        "Tienda de ropa juvenil", "Boutique de moda", "Zapatería", 
        "Tienda de zapatillas", "Sneakers", # <--- NUEVO (Para Jefa Sneakers)
        "Tienda de accesorios", "Joyería", "Tienda de deportes", "Ropa deportiva",
        "Tienda de ropa vintage", # <--- NUEVO (Clave en Justicia)

        # --- HOSTELERÍA & OCIO ---
        "Restaurante", "Restaurante de moda", "Bar", "Bar de tapas", 
        "Bar de copas", "Pub", 
        "Bar gay", "Bar LGTB", # <--- NUEVO (Para LL Bar y ambiente Chueca)
        "Cervecería", "Vinoteca", "Coctelería", # <--- NUEVO (Para Mojito's)
        "Cafetería", "Cafetería de especialidad", "Brunch", 
        "Heladería", "Ice Cream", # <--- NUEVO (Para Mistura)
        "Comida rápida", "Hamburguesería", "Pizzería", 
        "Comida saludable", "Restaurante vegano",

        # --- ALIMENTACIÓN & SERVICIOS ---
        "Supermercado", "Tienda de alimentación", "Mercado gastronómico", 
        "Panadería", "Pastelería", "Confitería",
        "Carnicería", "Frutería", "Tienda gourmet", 
        "Farmacia", "Centro comercial", "Tienda de cosméticos"
    ]

# ================= 2. GENERADOR DE MALLA FINA (HIGH DENSITY) =================
def generate_grid_points(polygon, step_degrees=0.002): 
    # ⚠️ AQUI ESTÁ LA CLAVE: 0.002
    # Esto reduce el tamaño del cuadrado a la mitad, generando 4 VECES MÁS PUNTOS.
    # Garantiza que capturemos todo en calles densas.
...
# ================= 3. OBTENCIÓN DE TARGETS =================
def get_study_targets(study_name):
    engine = create_engine(DB_CONNECTION_STR)
    
    # Mantenemos filtro Justicia y FIX Coordenadas 4326
    query = text(f"""
    ...
    try:
        ...
        if gdf.empty: 
            print("⚠️ No se encontró la zona.")
            return []
        
        search_targets = []
        # FORZAMOS EL PASO FINO AQUÍ (0.002) IGNORANDO EL CONFIG
        step = 0.002 
        ...
        for idx, row in gdf.iterrows():
            ...
            print(f"   📐 Zona '{zone_name}': {len(points)} puntos de MALLA FINA (High Precision).")
...
def run_study_scanner(study_name):
    ...
    filename = os.path.join(base_dir, f"{study_name}_RAW_V3.json") # Archivo V3
    checkpoint_file = os.path.join(base_dir, f"{study_name}_checkpoint_V3.json")
    ...
    print("-" * 60)
    print(f"💰 MODO COBERTURA TOTAL V3")
    print(f"   Archivo Salida: {os.path.basename(filename)}")
    print(f"   Puntos (Grid Fino): {len(targets)} (Alta densidad)")
    print(f"   Términos: {len(search_terms)} (Con Heladerías, Sneakers...)")
    ...
    if os.path.exists(filename):
        try:
            ...
            print(f"📚 Datos previos V3 recuperados: {len(master_db)}")
        except: pass
    ...
    print(f"\n✅ Finalizado V3. Total locales únicos capturados: {len(master_db)}")
