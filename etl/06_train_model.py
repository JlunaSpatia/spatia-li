...
def train_final_model_hardcore():
    print("🧠 ENTRENAMIENTO FINAL (MODO DESPIADADO V2)...")
    ...
    if train_df.empty: 
        print("❌ Error: No hay tiendas de entrenamiento.")
        return

    # Vector Ideal
    ideal_vector = train_df[weighted_cols].mean().values.reshape(1, -1)
    
    # Análisis de Umbral de Renta
    min_income_threshold = train_df['income_smooth'].min() * 0.9 
    print(f"\n💰 UMBRAL DE RENTA: {min_income_threshold:.0f}€")
    ...
    # --- AQUÍ ESTÁ EL ARREGLO: INCLUIMOS dist_transit ---
    final_cols = [
        'h3_index', 'city', 'lat', 'lon', 'similarity', 'geom',
        'dist_cafe', 'dist_gym', 'dist_shop', 'dist_transit', # <--- ¡AÑADIDO!
